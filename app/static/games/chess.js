const chessView = document.getElementById("chessView");
...
function renderBoard(player) {
  ...
  const rankLabels = player === 2 ? [1, 2, 3, 4, 5, 6, 7, 8] : [8, 7, 6, 5, 4, 3, 2, 1];
...
function applyState(state) {
  ...
  if (gameMode === "seats" && state.seat_info) {
    ...
    if (roleLine) {
      if (seatPlayer === 1) {
        roleLine.textContent = "role://seat 1 (white)";
      } else if (seatPlayer === 2) {
        roleLine.textContent = "role://seat 2 (black)";
      } else {
...
async function submitMove(from, to) {
  try {
    ...
    const response = await fetch(`/move?game=${getGameQuery()}`, {
      method: "POST",
...
async function requestReset() {
  try {
    ...
    const response = await fetch(`/reset?game=${getGameQuery()}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        player: viewPlayer,
